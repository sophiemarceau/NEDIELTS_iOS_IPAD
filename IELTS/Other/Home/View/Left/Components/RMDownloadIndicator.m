...
#define kXdf_ClosedIndicator 4.0
...
- (void)initAttributes
{
    ...
    if(_type == kRMClosedIndicator)
    {
        ...
        _closedIndicatorBackgroundStrokeColor = kIndicatorColor;
        _coverWidth = kXdf_ClosedIndicator;
...
{
     _titleLabel = [[UILabel alloc]initWithFrame:CGRectMake((CGRectGetWidth(self.bounds)/2 - 80/2), (CGRectGetHeight(self.bounds)/2 - 30/2)-5, 80, 30)];
...
//     titleLabel.text = self.titleLabelText;
    ...
    self.displayLabel = [[RMDisplayLabel alloc] initWithFrame:CGRectMake((CGRectGetWidth(self.bounds)/2 - 60/2), (CGRectGetHeight(self.bounds)/2 - 30/2)+10, 60, 30)];
    self.displayLabel.backgroundColor = [UIColor clearColor];
    self.displayLabel.font = [UIFont fontWithName:@"HelveticaNeue-Bold" size:16];
...
{
    if (_titleLabelText != titleLabelText) {
